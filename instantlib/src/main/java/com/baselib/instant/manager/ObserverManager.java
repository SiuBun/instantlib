 ...
 * @author wsb
 */
public class ObserverManager implements com.baselib.manager.IManager {
    ...
    public AppChangeObserver getAppChangeObserver() {
        IObserver observer = mObserverMap.get(APP_CHANGE_OBSERVER_NAME);
        if (observer == null) {
            LogUtils.d("该观察者暂未被支持");
        }
        return observer == null ? null : (AppChangeObserver) observer;
    }
}
